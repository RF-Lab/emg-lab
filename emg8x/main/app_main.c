...
int num_phys_channels = -1 ; // Number of physical channel on chip
int phys_block_size = -1 ; // Number of bits to read from device
...
IRAM_ATTR uint8_t ad1299_rreg(spi_device_handle_t spi, const uint8_t addr)
{
    ...
    ESP_ERROR_CHECK(ret) ;                          //Should have had no issues.
    ESP_LOGI( TAG, "ad1299_rreg: read from addr:0x%02X -> 0x%02X", addr,t.rx_data[0] ) ;
...
IRAM_ATTR void ad1299_read_data_block216(spi_device_handle_t spi, uint8_t* data216)
{
    ...
    t.length        = phys_block_size ; //216 ;     //Total data length, in bits
    t.rxlength      = phys_block_size ; //216 ;     //Total data length received, should be not greater than ``length`` in full-duplex mode (0 defaults this to the value of ``length``)
    t.user          = (void*)0 ;                    //User-defined variable. Can be used to store eg transaction ID.
...
//    int dmaChan             = 0 ;  // disable dma
    ...
    ESP_LOGI(TAG, "Wait for 2^18 tclk") ;

    vTaskDelay( 500 / portTICK_PERIOD_MS ) ;
    //ets_delay_us( 15*10 ) ; //~30 clock periods @2MHz

    // Reset pulse
    ESP_LOGI(TAG, "Reset ad129x") ;
    ...
    */
    ...
    // RREG id
    // ID Control register format:
    // 7 6 5 4 3 2 1 0
    // 1 0 0 1 0 x x x  - ADS129x device family
    // 1 1 0 1 0 x x x  - ADS129xR device family
    // ? ? ? 1 1 1 x x  - ADS1299-x device family 

    ESP_LOGI(TAG, "Read chip Id from Reg#0:") ;
    uint8_t value8              = ad1299_rreg( spi_dev, AD1299_ADDR_ID ) ;\

    uint8_t ads_check_bit    = (value8>>4) & 0x01 ;
    uint8_t ads_family_bit   = (value8>>3) & 0x01 ;

    if ( ads_check_bit && ads_family_bit )
    {        
        uint8_t ad1299_rev_id       =  value8>>5 ;
        uint8_t ad1299_dev_id       = (value8>>2) & 0x03 ;
        uint8_t ad1299_num_ch       = (value8) & 0x03 ;

        ESP_LOGI(TAG,"ads1299-x found:") ;
        ESP_LOGI(TAG, "!-->ad1299_rev_id:       0x%02X",  ad1299_rev_id ) ;
        ESP_LOGI(TAG, "!-->ad1299_check_bit:    %1d (should be 1)",  ads_check_bit ) ;
        ...
        else if (ad1299_num_ch==1)
        {
            num_phys_channels = 6 ;
        }
        else if (ad1299_num_ch==2)
        {
            num_phys_channels = 8 ;
        }
        else
        {
            ESP_LOGE(TAG, "error: ubnormal ad129x_num_ch!" ) ;
            return ;
        }
    }
    else if (ads_check_bit)
    {
        uint8_t ad129x_num_ch       = (value8) & 0x07 ;
        ESP_LOGI(TAG,"ADS129x found:") ;
        ESP_LOGI(TAG, "!-->ad129x_num_ch:       0x%02X",  ad129x_num_ch ) ;
        if (ad129x_num_ch==0)
        {
            num_phys_channels = 4 ;
        }
        else if (ad129x_num_ch==1)
        {
            num_phys_channels = 6 ;
        }
        else if (ad129x_num_ch==2)
        {
            num_phys_channels = 8 ;
        }
        else
    ...
    {
        ESP_LOGE(TAG, "error: ads1299-x, ads129x not found!" ) ;
        return ;
    }

    phys_block_size = 24*(num_phys_channels+1) ;

    ESP_LOGI(TAG, "num_phys_channels: %d", num_phys_channels ) ;
    ESP_LOGI(TAG, "phys_block_size: %d bits", phys_block_size ) ;
 ...
 /*   while(1)
    ...
    ad1299_wreg( spi_dev, AD1299_ADDR_CONFIG1, 0xf6 ) ;     // 0xf4 for 1299-x
    vTaskDelay( 100 / portTICK_PERIOD_MS ) ;
