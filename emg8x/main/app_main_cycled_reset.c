...
int adc_numch = -1 ; // Number of channels readed from configuration register 0
int adc_block_len = -1 ;
...
/*
    ad1299_read_data_block(spi_device_handle_t spi, uint8_t* data216, )
    spi     - spi handle
    data - pointer to array of 27 bytes length (216 bits=24bit * 9)
*/
IRAM_ATTR void ad1299_read_data_block(spi_device_handle_t spi, uint8_t* data, size_t dataSize)
{
    esp_err_t ret ;
    spi_transaction_t t ;
    memset( &t, 0, sizeof(spi_transaction_t) ) ;    //Zero out the transaction
    t.flags         = 0 ;                           //Bitwise OR of SPI_TRANS_* flags
    t.length        = dataSize ;//216 ;             //Total data length, in bits
    t.rxlength      = dataSize ;//216 ;             //Total data length received, should be not greater than ``length`` in full-duplex mode (0 defaults this to the value of ``length``)
    t.user          = (void*)0 ;                    //User-defined variable. Can be used to store eg transaction ID.
    t.tx_buffer     = NULL ;                        // There are No tx data to send
    t.rx_buffer     = data ;                     // Pointer to Rx buffer
...
//    int dmaChan             = 0 ;  // disable dma
    ...
    */
    ...
    while(true)
    {
        ...
        {
            adc_numch = 4 ;
            adc_block_len = 120 ; // 24*5
        }
        else if (ad1299_num_ch==1)
        {
            adc_numch = 6 ;
            adc_block_len = 168 ; // 24*7
        }
        else if (ad1299_num_ch==2)
        {
            adc_numch = 8 ;
            adc_block_len = 216 ; // 24*9
        }
        else
        {
            ESP_LOGE(TAG, "error: ads1299 wrong num_channel descriptor!" ) ;
        ...
        if (ad1299_check_bit && ad1299_dev_id==0x03 )
        {
            ...
            ESP_LOGI(TAG, "!-->ad129x_rev_id:       0x%02X",  ad1299_rev_id ) ;
            ESP_LOGI(TAG, "!-->ad129x_check_bit:    %1d (should be 1)",  ad1299_check_bit ) ;
            ESP_LOGI(TAG, "!-->ad129x_dev_id:       0x%02X",  ad1299_dev_id ) ;
            ESP_LOGI(TAG, "!-->ad129x_num_ch:       %1d",  adc_numch ) ;
            ESP_LOGI(TAG, "!-->ad129x_block_len:    %3d",  adc_block_len ) ;
        }
        else
        {
            ESP_LOGE(TAG, "error: ads129x not found!" ) ;
            vTaskDelay( 2500 / portTICK_PERIOD_MS ) ;
            continue ;
        }
 ...
 /*   while(1)
    ...
    for(int nSample=0;nSample<CONFIG_EMG8X_SAMPLES_PER_TRANSPORT_BLOCK;nSample++)
    {
        ...
        // DRDY goes down - data ready to read
        ad1299_read_data_block( spi_dev, rxDataBuf, adc_block_len ) ;
...
IRAM_ATTR void spi_data_pump_task( void* pvParameter )
{
    ...
    while(1)
    {
        ...
        ad1299_read_data_block( spi_dev, drdy_thread_context.spiReadBuffer, adc_block_len ) ;
#endif
        //gpio_set_level( DEBUG_PIN1,     0 ) ;
